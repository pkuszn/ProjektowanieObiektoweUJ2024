...
public class Test_Simulation extends TestBase {
        ...
        private void test_simulation_complex_circuit()
                        throws UnknownChip, UnknownStateException, UnknownPin, ShortCircuitException, UnknownComponent {
                this.simulationManager.resetComponents();
                this.componentManager.resetComponents();
                ...
                // checkPinState(result);
                for (int i = 0; i <= tick; i++) {
                        if (i == 0) {
                                try {
                                        PinState state1 = result.get(i).stream()
                                                        .filter(state -> state.componentId() == chipOut1
                                                                        && state.pinId() == 1)
                                                        .findFirst().orElseThrow().state();

                                        assert verifyOutput(state1, PinState.HIGH) == true;
                                        System.out.println(okMessage(this.getClass().getSimpleName(),
                                                        this.getCurrentMethodName(),
                                                        "tick: " + i + " pinNumber: " + 1));

                                } catch (AssertionError e) {
                                        System.out.println(
                                                        failedMessage(this.getClass().getSimpleName(),
                                                                        this.getCurrentMethodName(), e.getMessage()));
                                }
        ...
        private void test_simulation_max_complex_circuit()
                        throws UnknownChip, UnknownStateException, UnknownPin, ShortCircuitException, UnknownComponent {
                this.simulationManager.resetComponents();
                this.componentManager.resetComponents();
                ...
                result.forEach((key, value) -> {
                        ...
                        for (ComponentPinState c : value)
                                if (c.state() != PinState.UNKNOWN)
                ...
                for (int i = 0; i <= tick; i++) {
                        ...
                        try {
                                ...
                                System.out.println(okMessage(this.getClass().getSimpleName(),
                                                this.getCurrentMethodName(), "tick: " + i + " pinNumber: " + 2));
